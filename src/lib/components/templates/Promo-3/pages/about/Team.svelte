<script lang="ts">
	...
	const team = $derived(
		Array.isArray(data?.team) && (data.team as unknown[]).length > 0
			? (data.team as typeof defaultTeam)
			: defaultTeam
	);
</script>
...
<section class="relative overflow-hidden bg-surface-800/30 py-24 lg:py-32">
	<div class="px-10 lg:px-24 xl:px-32">
		<div class="mb-16 text-center">
			...
			<EditableField
				fieldKey="Team.heading"
				label="Заголовок"
				value={String(data?.heading ?? 'Команда')}
				{isEditable}
			...
			<EditableField
				...
				class="block"
			>
				{#snippet children(displayValue)}<p class="mx-auto mt-4 max-w-xl text-surface-300">
						{displayValue}
					</p>{/snippet}
			</EditableField>
		</div>
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
			{#each team as member}
				...
				>
					...
					<h3 class="text-lg text-white">{member.name}</h3>
