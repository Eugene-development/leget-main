...
<section class="relative overflow-hidden py-24 lg:py-32">
	...
	<div class="relative px-8 text-center lg:px-16 xl:px-20">
		<EditableField
			...
			label="Заголовок"
			value={String(data?.heading ?? 'Готовы начать вместе?')}
			{isEditable}
			onSave={(v) => saveField('heading', v)}
			class="block"
		>
			{#snippet children(displayValue)}<h2 class="text-4xl text-white lg:text-5xl">
					{displayValue}
				</h2>{/snippet}
		</EditableField>
		<EditableField
			...
			value={String(
				data?.description ??
					'Свяжитесь с нами — поможем подобрать идеальную плитку, рассчитаем количество и сделаем 3D-визуализацию бесплатно.'
			)}
			{isEditable}
			multiline
		...
		<div class="mt-10 flex flex-wrap items-center justify-center gap-4">
			<EditableField
				...
				label="Кнопка 1"
				value={String(data?.ctaPrimary ?? 'Смотреть каталог')}
				{isEditable}
				onSave={(v) => saveField('ctaPrimary', v)}
			>
				{#snippet children(displayValue)}
					<a
						href={String(data?.ctaPrimaryLink ?? '/collections')}
			...
			<EditableField
				...
				label="Кнопка 2"
				value={String(data?.ctaSecondary ?? 'Связаться с нами')}
				{isEditable}
