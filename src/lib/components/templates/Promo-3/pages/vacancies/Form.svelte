<script lang="ts">
	// Артикул: 3.8.3.1 — см. docs/architecture/component-articles-map.md
	...
	<div class="relative px-10 lg:px-24 xl:px-32">
		<div class="mx-auto max-w-2xl">
			<div class="mb-12 text-center">
				...
				<EditableField
					fieldKey="Form.heading"
					label="Заголовок"
					value={String(data?.heading ?? 'Напишите нам')}
					{isEditable}
				...
				<EditableField
					...
					class="block"
				>
					{#snippet children(displayValue)}<p class="mt-4 text-surface-300">
							{displayValue}
						</p>{/snippet}
				</EditableField>
			</div>
			...
			>
				<div
					class="absolute -top-px right-12 h-px w-24 bg-linear-to-r from-transparent via-accent-500 to-transparent"
				></div>
				<div
					class="absolute -bottom-px left-12 h-px w-24 bg-linear-to-r from-transparent via-accent-500/50 to-transparent"
				></div>
				...
				{#if submitted}
					<div class="flex flex-col items-center py-8 text-center">
						...
						<h3 class="ds-title-sub text-2xl text-white">Отклик получен!</h3>
						<p class="mt-3 text-surface-300">
							Мы рассмотрим вашу кандидатуру и свяжемся с вами в течение <span
								class="font-semibold text-accent-500">1 рабочего дня</span
							>.
						</p>
						<button
							onclick={resetForm}
					...
					<form onsubmit={handleApply} class="flex flex-col gap-6">
						<div class="flex flex-col gap-2">
							...
							<input
								...
								class="w-full rounded-xl border border-surface-600 bg-surface-700/50 px-5 py-4 text-sm text-white placeholder-surface-300 transition-all duration-300 outline-none focus:border-accent-500/60 focus:bg-surface-700"
							/>
						</div>
						<div class="flex flex-col gap-2">
							...
							{#if loading}
								<span class="flex items-center justify-center gap-3"
									><svg
										...
										><circle cx="12" cy="12" r="10" stroke-opacity="0.3" /><path
