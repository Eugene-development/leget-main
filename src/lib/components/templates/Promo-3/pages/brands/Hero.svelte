	...
	<div class="relative z-10 w-full px-10 pt-40 pb-20 lg:px-24 xl:px-32">
		...
		>
			...
			<EditableField
				fieldKey="Hero.tag"
				label="Тег"
				value={String(data?.tag ?? 'Наши партнёры')}
				{isEditable}
		...
		<EditableField
			fieldKey="Hero.title"
			label="Заголовок"
			value={String(data?.title ?? 'Бренды')}
			{isEditable}
		...
		<EditableField
			...
			value={String(
				data?.description ??
					'Прямые поставки от ведущих производителей плитки из России, Италии, Испании и всей Европы.'
			)}
			{isEditable}
			multiline
