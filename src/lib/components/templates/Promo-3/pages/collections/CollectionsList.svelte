...
<section class="relative py-24 lg:py-32">
	<div class="px-10 lg:px-24 xl:px-32">
		...
		<div class="mb-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
			<div class="relative w-full max-w-sm">
				...
				<input
					...
					class="w-full rounded-xl border border-surface-600 bg-surface-800/50 py-3.5 pr-5 pl-11 text-sm text-white placeholder-surface-400 transition-all duration-300 outline-none focus:border-accent-500/60 focus:bg-surface-800"
				/>
			</div>
		...
		<p class="mb-8 text-sm text-surface-400">
			{#if filteredCollections.length === collections.length}Все {collections.length} коллекций{:else}Найдено:
				<span class="text-accent-500">{filteredCollections.length}</span> из {collections.length}{/if}
		</p>
				...
				{#each filteredCollections as item}
					...
					>
						...
						<div class="flex flex-1 flex-col p-5">
							...
							<h2
								class="mt-1.5 text-base text-white transition-colors duration-300 group-hover:text-accent-400"
							>
								{item.name}
							</h2>
							<div
