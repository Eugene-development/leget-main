...
<section class="relative flex min-h-screen items-center overflow-hidden">
	...
	<div class="relative z-10 px-10 pt-40 pb-24 lg:px-24 lg:pt-48 lg:pb-32 xl:px-32">
		...
		<div class="mb-8 inline-flex items-center gap-2 rounded-full border border-accent-500/20 bg-accent-500/5 px-4 py-1.5">
			...
			<EditableField fieldKey="Hero.tag" label="Тег" value={String(data?.tag ?? 'Коллекция 2026')} {isEditable} onSave={(v) => saveField('tag', v)}>
		...
		<!-- Heading -->
		<EditableField fieldKey="Hero.title" label="Заголовок" value={String(data?.title ?? 'Искусство плитки в каждой детали')} {isEditable} onSave={(v) => saveField('title', v)} class="block">
			{#snippet children(displayValue)}
		...
		<!-- Subtitle -->
		<EditableField fieldKey="Hero.subtitle" label="Подзаголовок" value={String(data?.subtitle ?? 'Качественная плитка из разных материалов от ведущих мировых производителей.')} {isEditable} multiline onSave={(v) => saveField('subtitle', v)} class="block">
			{#snippet children(displayValue)}
		...
		<div class="mt-10 flex flex-wrap items-center gap-4">
			<EditableField fieldKey="Hero.ctaPrimary" label="Кнопка 1" value={String(data?.ctaPrimary ?? 'Промокод на скидку 10%')} {isEditable} onSave={(v) => saveField('ctaPrimary', v)}>
				{#snippet children(displayValue)}
					<a href={String(data?.ctaPrimaryLink ?? '/contacts')} class="group inline-flex items-center gap-3 rounded-xl bg-accent-500 px-8 py-4 text-sm font-bold tracking-wide text-surface-900 uppercase transition-all duration-300 hover:bg-accent-400 hover:shadow-2xl hover:shadow-accent-500/30">
						{displayValue}
			...
			<EditableField fieldKey="Hero.ctaSecondary" label="Кнопка 2" value={String(data?.ctaSecondary ?? 'Курьер с образцом')} {isEditable} onSave={(v) => saveField('ctaSecondary', v)}>
				{#snippet children(displayValue)}
