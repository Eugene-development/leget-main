...
<section class="relative bg-surface-800/30 py-24 lg:py-32">
	<div class="px-10 lg:px-24 xl:px-32">
		<div class="mb-16 text-center">
			<EditableField fieldKey="Collections.label" label="Лейбл" value={String(data?.label ?? 'Популярное')} {isEditable} onSave={(v) => saveField('label', v)}>
				{#snippet children(displayValue)}
					<span class="text-xs font-semibold tracking-[0.3em] text-accent-500 uppercase">{displayValue}</span>
				{/snippet}
			</EditableField>
			<EditableField fieldKey="Collections.heading" label="Заголовок" value={String(data?.heading ?? 'Бестселлеры коллекций')} {isEditable} onSave={(v) => saveField('heading', v)} class="block">
				{#snippet children(displayValue)}
					<h2 class="mt-3 font-display text-4xl font-bold tracking-wide text-white lg:text-5xl">{displayValue}</h2>
				{/snippet}
			</EditableField>
			<EditableField fieldKey="Collections.description" label="Описание" value={String(data?.description ?? 'Самые востребованные коллекции плитки, выбранные нашими клиентами и профессиональными дизайнерами')} {isEditable} multiline onSave={(v) => saveField('description', v)} class="block">
				{#snippet children(displayValue)}
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each collections as item}
				<div class="group relative overflow-hidden rounded-2xl border border-surface-700/50 bg-surface-800/80 transition-all duration-500 hover:border-accent-500/20 hover:shadow-2xl hover:shadow-accent-500/5">
					...
					<div class="p-5">
						<div class="text-[11px] font-medium tracking-wider text-surface-300 uppercase">{item.brand}</div>
						<h3 class="mt-1 text-lg font-bold text-white transition-colors group-hover:text-accent-500">{item.name}</h3>
						<div class="mt-3 flex items-center justify-between">
							<span class="font-display text-lg font-bold text-accent-500">{item.price}</span>
							<button class="flex items-center gap-2 rounded-lg border border-surface-600 bg-surface-700/50 px-4 py-2 text-xs font-medium text-surface-300 transition-all duration-300 hover:border-accent-500/40 hover:text-accent-500">
								Подробнее
								<svg viewBox="0 0 20 20" class="size-3.5 fill-current"><path fill-rule="evenodd" d="M3 10a.75.75 0 01.75-.75h10.638L10.23 5.29a.75.75 0 111.04-1.08l5.5 5.25a.75.75 0 010 1.08l-5.5 5.25a.75.75 0 11-1.04-1.08l4.158-3.96H3.75A.75.75 0 013 10z" clip-rule="evenodd" /></svg>
							</button>
						</div>
					</div>
				</div>
