<script lang="ts">
	// Артикул: 3.1.6.1 — см. docs/architecture/component-articles-map.md
...
<section class="relative overflow-hidden py-24 lg:py-32">
	...
	<div class="relative px-8 text-center lg:px-16 xl:px-20">
		<EditableField
			...
			label="Заголовок"
			value={String(data?.heading ?? 'Готовы начать проект?')}
			{isEditable}
			onSave={(v) => saveField('heading', v)}
			class="block"
		>
			{#snippet children(displayValue)}
				<h2 class="font-display text-4xl text-white lg:text-5xl">{displayValue}</h2>
			{/snippet}
		</EditableField>
		<EditableField
			...
			value={String(
				data?.description ??
					'Свяжитесь с нами для бесплатной консультации. Наши специалисты помогут подобрать идеальную плитку для вашего интерьера.'
			)}
			{isEditable}
			multiline
		...
		<div class="mt-10 flex flex-wrap items-center justify-center gap-4">
			<EditableField
				...
				label="Кнопка 1"
				value={String(data?.ctaPrimary ?? 'Позвонить нам')}
				{isEditable}
			...
			<EditableField
				...
				label="Кнопка 2"
				value={String(data?.ctaSecondary ?? 'Оставить заявку')}
				{isEditable}
