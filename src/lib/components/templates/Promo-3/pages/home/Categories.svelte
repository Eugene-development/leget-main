...
<section class="relative py-24 lg:py-32">
	<div class="px-10 lg:px-24 xl:px-32">
		<div class="mb-16 flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
			<div>
				<EditableField
					fieldKey="Categories.label"
					label="Лейбл"
					value={String(data?.label ?? 'Категории')}
					{isEditable}
				...
				<EditableField
					...
					label="Заголовок"
					value={String(data?.heading ?? 'Подберите идеальную плитку')}
					{isEditable}
			...
			<a
				href={String(data?.allLink ?? '/collections')}
				...
				>
					...
					<div class="absolute right-0 bottom-0 left-0 p-6">
						<span
							...
							>{cat.count}</span
						>
						<h3 class="text-lg text-white">{cat.title}</h3>
