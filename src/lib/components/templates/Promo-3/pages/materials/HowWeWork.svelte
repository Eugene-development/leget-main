<script lang="ts">
	// Артикул: 3.6.3.1 — см. docs/architecture/component-articles-map.md
	...
	const defaultSteps = [
		{
			step: '01',
			title: 'Консультация',
			desc: 'Обсуждаем ваш проект, замеры и предпочтения по дизайну'
		},
		{
			step: '02',
			title: 'Подбор материала',
			desc: 'Предлагаем оптимальные варианты с учётом бюджета и стиля'
		},
		{
			step: '03',
			title: 'Расчёт количества',
			desc: 'Точно рассчитываем необходимое количество с учётом раскладки и обрезков'
		},
		{
			step: '04',
			title: 'Доставка и укладка',
			desc: 'Организуем доставку и при желании — укладку с нашими мастерами'
		}
	];
	const steps = $derived(
...
<section class="relative bg-surface-800/30 py-24 lg:py-32">
	...
	<div class="relative px-10 lg:px-24 xl:px-32">
		<div class="mb-14 text-center">
			<EditableField
				fieldKey="HowWeWork.heading"
				label="Заголовок"
				value={String(data?.heading ?? 'Как мы работаем')}
				{isEditable}
			...
			<EditableField
				fieldKey="HowWeWork.description"
				label="Описание"
				value={String(data?.description ?? 'Простой процесс от выбора до укладки')}
				{isEditable}
				onSave={(v) => saveField('description', v)}
				class="block"
			>
				{#snippet children(displayValue)}<p class="mt-4 text-surface-400">
						{displayValue}
					</p>{/snippet}
			</EditableField>
		</div>
