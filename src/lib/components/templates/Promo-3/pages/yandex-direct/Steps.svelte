<script lang="ts">
	// Артикул: 3.9.4.1 — см. docs/architecture/component-articles-map.md
...
<section class="relative overflow-hidden py-24 lg:py-32">
	<div
		class="absolute inset-0 bg-linear-to-br from-accent-600/5 via-surface-900 to-surface-900"
	></div>
	<div
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
			{#each steps as step, i}
				...
				>
					...
					{#if i < steps.length - 1}
						<div class="absolute top-1/2 -right-3 z-10 hidden -translate-y-1/2 lg:block">
							<svg
								class="size-6 text-accent-500/30"
								fill="none"
								viewBox="0 0 24 24"
								stroke="currentColor"
								stroke-width="1.5"
					...
					<h3 class="text-lg text-white">{step.title}</h3>
