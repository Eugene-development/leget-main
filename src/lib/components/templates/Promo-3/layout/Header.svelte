<script lang="ts">
	...
	const siteTagline = $derived(
		typeof data?.siteTagline === 'string' ? data.siteTagline : 'Интерьер & Дизайн'
	);
	...
	function updateCounts() {
		if (!browser) return;
		try {
			favoritesCount = JSON.parse(localStorage.getItem('plitka_favorites') ?? '[]').length;
		} catch {
			favoritesCount = 0;
		}
		try {
			compareCount = JSON.parse(localStorage.getItem('plitka_compare') ?? '[]').length;
		} catch {
			compareCount = 0;
		}
		try {
			...
			cartCount = ca
				? (JSON.parse(ca) as { qty?: number }[]).reduce((s, i) => s + (i.qty ?? 1), 0)
				: 0;
		} catch {
			cartCount = 0;
		}
	...
	{#if mobileMenuOpen}
		...
		>
			...
			{#if phone}
				<a
					href="tel:{phone}"
					class="flex items-center gap-3 rounded-xl border border-surface-700/50 bg-black/20 px-6 py-3 text-lg font-medium text-surface-100 backdrop-blur-xl transition-all hover:bg-black/30 hover:text-accent-400"
				>
					<svg
						viewBox="0 0 24 24"
						class="size-5 fill-none stroke-accent-500 stroke-2"
			...
			<button
				...
				class="text-sm text-surface-300 transition-colors hover:text-surface-50"
			>
				{$auth.isAuthenticated ? 'Выйти' : 'Войти'}
			</button>
		</div>
