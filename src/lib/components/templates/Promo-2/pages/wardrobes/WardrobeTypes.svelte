	...
	const defaultTypes = [
		{
			...
			description:
				'Монтируются от пола до потолка, скрывая неровности стен и максимально эффективно используя ниши',
			image: null,
			offset: ''
		},
		{
			...
			description:
				'Самостоятельные модули, которые можно перемещать. Акцентные фасады и витринные стеклянные элементы',
			image: null,
...
<section
	bind:this={sectionEl}
	class="overflow-hidden bg-white px-6 py-section-sm lg:py-section xl:px-1"
>
	<div class="mx-auto max-w-screen-xl">
		<div
			class="mb-16 flex flex-col items-end justify-between gap-8 opacity-0 transition-all duration-700 lg:flex-row"
		...
		<div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
			{#each types as type, i}
				...
				>
					...
					<h3 class="mb-2 text-xl text-primary" style="font-family: var(--font-heading);">
						{type.title}
					</h3>
