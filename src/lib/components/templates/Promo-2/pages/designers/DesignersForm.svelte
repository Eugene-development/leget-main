...
<section bind:this={sectionEl} class="relative overflow-hidden bg-primary py-section-sm lg:py-section" id="application">
		...
		<div class="grid gap-16 lg:grid-cols-2 lg:gap-24">
			...
			<div class="opacity-0 transition-all duration-700" class:animate-fade-up={visible}>
				...
				<EditableField
					...
					label="Описание"
					value={String(data?.description ?? 'Расскажите о себе, и мы предложим наиболее подходящие условия сотрудничества.')}
					{isEditable}
					multiline
			...
			<div class="opacity-0 transition-all duration-700" class:animate-fade-up={visible} style="animation-delay: 0.2s">
				{#if formSent}
					<div class="flex h-full flex-col items-center justify-center py-12 text-center">
						...
						<h3 class="mt-6 text-2xl font-light text-white" style="font-family: var(--font-heading);">Заявка отправлена</h3>
						<p class="mt-3 max-w-xs text-sm text-white/50">Менеджер свяжется с вами в течение одного рабочего дня.</p>
					</div>
				{:else}
					<form onsubmit={handleSubmit} class="border border-white/10 bg-white/5 p-8 backdrop-blur-sm lg:p-10">
						<h3 class="mb-8 text-xl font-light text-white" style="font-family: var(--font-heading);">Заявка на партнёрство</h3>

						<!-- Type -->
						<div class="mb-6">
							<p class="mb-3 text-[11px] tracking-[0.2em] text-white/40 uppercase">Я работаю как</p>
							<div class="flex gap-3">
								<label class="flex flex-1 cursor-pointer items-center gap-3 rounded-sm border px-4 py-3 transition-colors duration-300 {formType === 'studio' ? 'border-accent bg-accent/10' : 'border-white/10 hover:border-white/25'}">
									<input type="radio" bind:group={formType} value="studio" class="sr-only" />
									<span class="flex h-4 w-4 shrink-0 items-center justify-center border {formType === 'studio' ? 'border-accent' : 'border-white/30'}">
										{#if formType === 'studio'}<span class="h-2 w-2 bg-accent"></span>{/if}
									</span>
									<span class="text-sm text-white/80">Студия</span>
								</label>
								<label class="flex flex-1 cursor-pointer items-center gap-3 rounded-sm border px-4 py-3 transition-colors duration-300 {formType === 'freelance' ? 'border-accent bg-accent/10' : 'border-white/10 hover:border-white/25'}">
									<input type="radio" bind:group={formType} value="freelance" class="sr-only" />
									<span class="flex h-4 w-4 shrink-0 items-center justify-center border {formType === 'freelance' ? 'border-accent' : 'border-white/30'}">
						...
						<div class="mb-5">
							<label for="form-name" class="mb-2 block text-[11px] tracking-[0.2em] text-white/40 uppercase">Имя и фамилия*</label>
							<input id="form-name" type="text" bind:value={formName} required placeholder="Анна Смирнова" class="w-full border border-white/15 bg-transparent px-4 py-3 text-sm text-white placeholder-white/25 transition-colors duration-300 outline-none focus:border-accent/60" />
						</div>

						<div class="mb-5">
							<label for="form-phone" class="mb-2 block text-[11px] tracking-[0.2em] text-white/40 uppercase">Телефон*</label>
							<input id="form-phone" type="tel" bind:value={formPhone} required placeholder="+7 (900) 000-00-00" class="w-full border border-white/15 bg-transparent px-4 py-3 text-sm text-white placeholder-white/25 transition-colors duration-300 outline-none focus:border-accent/60" />
						</div>

						<div class="mb-8">
							<label for="form-email" class="mb-2 block text-[11px] tracking-[0.2em] text-white/40 uppercase">Email*</label>
							<input id="form-email" type="email" bind:value={formEmail} required placeholder="your@email.com" class="w-full border border-white/15 bg-transparent px-4 py-3 text-sm text-white placeholder-white/25 transition-colors duration-300 outline-none focus:border-accent/60" />
						</div>
