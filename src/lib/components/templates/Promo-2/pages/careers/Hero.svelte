<script lang="ts">
	// Артикул: 2.12.1.1 — см. docs/architecture/component-articles-map.md
...
<section bind:this={sectionEl} class="relative overflow-hidden bg-primary py-28 lg:py-36">
	...
	></div>
	<div
		class="absolute top-0 left-0 h-px w-full bg-linear-to-r from-transparent via-secondary to-transparent opacity-30"
	></div>
		...
		<div class="max-w-2xl">
			...
			<div class="opacity-0" class:animate-fade-up={visible} style="animation-delay: 0.4s">
				...
				>
					{#snippet children(displayValue)}
						<h1
							class="mt-6 text-5xl text-white md:text-6xl lg:text-7xl"
			...
			<div class="opacity-0" class:animate-fade-up={visible} style="animation-delay: 0.6s">
				<EditableField
					...
					value={String(
						data?.description ??
							'Фабрика — это более 700 специалистов, которые создают премиальную мебель. Присоединяйтесь к команде профессионалов.'
					)}
					{isEditable}
					multiline
