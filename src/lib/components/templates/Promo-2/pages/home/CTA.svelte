	...
	<div class="mx-auto max-w-2xl">
		...
		>
			{#snippet children(displayValue)}
				<h2 class="text-3xl font-light sm:text-4xl">{displayValue}</h2>
			{/snippet}
		</EditableField>
		...
		>
			{#snippet children(displayValue)}
				<a
					href="/contact"
					class="mt-8 inline-block rounded-xl border border-white/30 bg-white/10 px-8 py-4 text-base font-medium text-white backdrop-blur-sm transition hover:bg-white/20"
				>
					{displayValue}
				</a>
			{/snippet}
