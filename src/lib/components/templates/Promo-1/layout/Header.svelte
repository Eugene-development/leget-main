<script lang="ts">
	...
	const catalogItems = [
		...
		{ href: '/stoleshnica', label: 'Столешницы', icon: 'surface', desc: 'Из камня, дерева и пластика' },
		{ href: '/bytovaya-tehnika', label: 'Бытовая техника', icon: 'fridge', desc: 'Встраиваемая и отдельностоящая' },
		{ href: '/santehnika', label: 'Сантехника', icon: 'faucet', desc: 'Смесители, мойки, аксессуары' },
	...
	const serviceItems = [
		{ href: '/consultation', label: 'Бесплатная консультация', icon: 'chat', desc: 'Профессиональная помощь в подборе решений' },
		{ href: '/design-project', label: 'Дизайн интерьера', icon: 'design', desc: 'Индивидуальные 3D-проекты любой сложности' },
		{ href: '/measurement', label: 'Замер помещения', icon: 'ruler', desc: 'Точные расчеты для идеальной установки' },
		{ href: '/furniture-project', label: 'Проект мебели', icon: 'project', desc: 'Проработка конструктива и эргономики' },
		{ href: '/assembly', label: 'Сборка и установка', icon: 'tools', desc: 'Качественный монтаж вашей мебели' }
	];
				...
				{#if link.label === 'Услуги'}
					<div role="group" class="relative flex items-center h-full" onmouseenter={() => (visibleServicesMenu = true)} onmouseleave={() => (visibleServicesMenu = false)}>
						...
						>
							...
							<svg 
								class="h-3.5 w-3.5 transition-transform duration-300 {visibleServicesMenu ? 'rotate-180 text-sky-500' : 'text-slate-400 group-hover:text-sky-500'}" 
								fill="none" 
								viewBox="0 0 24 24" 
								stroke="currentColor"
							>
						...
						{#if visibleServicesMenu}
							<div class="absolute left-1/2 top-full -translate-x-1/2 h-4 w-full"></div>
							<div
								transition:fly={{ y: -10, duration: 250, easing: cubicOut }}
								class="absolute left-1/2 top-full z-40 mt-4 w-80 -translate-x-1/2 overflow-hidden rounded-2xl bg-white/98 p-2 shadow-2xl ring-1 ring-slate-200 backdrop-blur-xl"
							>
								...
								{#each serviceItems as service, idx}
									...
									>
										<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-sky-100/50 to-indigo-100/50 text-sky-600 transition-all duration-300 group-hover:from-sky-500 group-hover:to-indigo-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-sky-500/25">
										...
										<div class="flex-1">
											<p class="text-sm font-bold text-slate-900 transition-colors group-hover:text-sky-600">
												{service.label}
											</p>
				...
				{:else if link.label === 'Каталог'}
					<div role="group" class="relative flex items-center h-full" onmouseenter={() => (visibleCatalogMenu = true)} onmouseleave={() => (visibleCatalogMenu = false)}>
						...
						>
							<span>{link.label}</span>
							<svg 
								class="h-3.5 w-3.5 transition-transform duration-300 {visibleCatalogMenu ? 'rotate-180 text-sky-500' : 'text-slate-400 group-hover:text-sky-500'}" 
								fill="none" 
								viewBox="0 0 24 24" 
								stroke="currentColor"
							>
	...
	{#if uiStore.menuOpen}
		<nav class="bg-white border-t border-slate-100 px-6 py-4 sm:hidden max-h-[70vh] overflow-y-auto shadow-2xl">
			{#each links as link}
				{#if link.label === 'Услуги'}
					<div class="py-2">
						<div class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">{link.label}</div>
