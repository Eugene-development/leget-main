...
{#snippet renderIcon(name: string)}
	...
	{:else if name === 'sofa'}
		<svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 12V8a2 2 0 00-2-2H6a2 2 0 00-2 2v4M4 12v6a2 2 0 002 2h2m10-8V8a2 2 0 00-2-2H6a2 2 0 00-2 2v4m16 0v6a2 2 0 01-2 2h-2M4 12h16m-16 0v-2a2 2 0 012-2h12a2 2 0 012 2v2" />
		</svg>
	{:else if name === 'surface'}
		<svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M3 14h18m-9-4v8m-3-8v8m6-8v8M3 6h18a2 2 0 012 2v8a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2z" />
		</svg>
	{:else if name === 'fridge'}
		<svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9h4m-4 4h4" />
		</svg>
	{:else if name === 'faucet'}
		<svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
	...
	{:else if name === 'hinge'}
		<svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
			<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m12 12a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />
		</svg>
	{:else if name === 'tiles'}
		<svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
{#snippet megaItem(
	...
	>
		...
		>
			...
			<span
				class="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-slate-100/80 ring-1 ring-slate-900/5 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover/item:bg-linear-to-br group-hover/item:from-sky-500 group-hover/item:to-indigo-500 group-hover/item:shadow-lg group-hover/item:shadow-sky-500/25"
			>
				<span
					class="text-slate-500 transition-colors duration-500 group-hover/item:text-white"
				>
			...
			<span class="min-w-0 flex-1">
				<span class="flex items-center gap-2">
					<span class="text-[13px] font-semibold tracking-tight text-slate-900 transition-colors duration-300 group-hover/item:text-sky-600">
						{item.label}
					</span>
					{#if isEditable && !isEnabled && !isComingSoon}
						<span class="rounded bg-slate-200 px-1.5 py-0.5 text-[9px] font-bold tracking-wider text-slate-600 uppercase">Откл.</span>
					{/if}
				</span>
				<span class="mt-0.5 block text-[11px] leading-snug text-slate-400">{item.desc}</span>
			</span>
			{#if !isEditable && !isComingSoon}
				<span
					class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100/0 text-slate-300 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover/item:translate-x-0.5 group-hover/item:bg-sky-50 group-hover/item:text-sky-500"
				>
		...
		{#if isEditable}
			{#if isComingSoon}
				<div class="flex shrink-0 items-center pr-3" title="В разработке">
					<span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-amber-50 text-amber-600 ring-1 ring-amber-200/80" aria-hidden="true">
			...
			{:else}
				<div class="flex items-center pr-3">
					<button
						...
						class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:outline-none {isEnabled ? 'bg-sky-500' : 'bg-slate-300'}"
						title={isEnabled ? toggleTitleOn : toggleTitleOff}
					>
						<span class="pointer-events-none relative inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out {isEnabled ? 'translate-x-4' : 'translate-x-0'}"></span>
					</button>
				</div>
			{/if}
		{/if}
...
{#snippet dropdownTrigger(label: string, active: boolean, open: boolean)}
	<button
		...
		class="group relative flex items-center gap-1.5 rounded-full px-4 py-2 text-[13px] font-semibold tracking-wide transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] {active
			...
			: 'text-slate-600 hover:bg-slate-900/4 hover:text-slate-900'}"
		aria-expanded={open}
	>
		<span>{label}</span>
		<svg
			class="h-3.5 w-3.5 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] {open ? 'rotate-180 text-sky-500' : 'text-slate-400'}"
			fill="none"
			viewBox="0 0 24 24"
			stroke="currentColor"
		>
			...
			>
				...
				{#if visibleServicesMenu}
					...
					<div
						...
						class="absolute top-full left-1/2 z-50 mt-4 w-88 -translate-x-1/2 rounded-[1.75rem] bg-slate-100/70 p-1.5 shadow-[0_28px_70px_-24px_rgba(15,23,42,0.45)] ring-1 ring-slate-900/5 backdrop-blur-2xl"
					>
						<div class="rounded-[calc(1.75rem-0.375rem)] bg-white/95 p-2 shadow-[inset_0_1px_1px_rgba(255,255,255,0.6)] ring-1 ring-slate-900/3">
							<div class="mb-1 flex items-center justify-between px-3 pt-2 pb-1">
								<span class="text-[10px] font-bold tracking-[0.22em] text-slate-400 uppercase">Услуги</span>
								<span class="h-px flex-1 ml-3 bg-linear-to-r from-slate-200 to-transparent"></span>
							</div>
			...
			>
				...
				{#if visibleCatalogMenu}
					...
					>
						<div class="rounded-[calc(1.75rem-0.375rem)] bg-white/95 p-2 shadow-[inset_0_1px_1px_rgba(255,255,255,0.6)] ring-1 ring-slate-900/3">
							<div class="mb-1 flex items-center justify-between px-3 pt-2 pb-1">
								<span class="text-[10px] font-bold tracking-[0.22em] text-slate-400 uppercase">Каталог</span>
								<span class="h-px flex-1 ml-3 bg-linear-to-r from-slate-200 to-transparent"></span>
							</div>
