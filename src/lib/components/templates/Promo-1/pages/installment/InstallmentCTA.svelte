<script lang="ts">
	// Артикул: 1.8.7.1 — см. docs/architecture/component-articles-map.md
...
<section class="relative isolate overflow-hidden bg-ink-950 py-24 sm:py-28 lg:py-32">
	<div class="pointer-events-none absolute inset-0" aria-hidden="true">
		...
		<div class="it-glow absolute -right-24 -bottom-40 size-112 bg-cat-4-400/15"></div>
		<div
			class="absolute inset-x-0 top-0 mx-auto h-px w-2/3 bg-linear-to-r from-transparent via-cat-3-400/60 to-transparent"
		></div>
		<div
				...
				<div class="relative">
					...
					>
						{#snippet children(displayValue)}
							<p
								class="inline-flex items-center gap-3 text-[11px] font-semibold tracking-[0.28em] text-cat-3-300 uppercase sm:text-xs"
							>
								<span class="h-px w-8 bg-cat-3-400/80" aria-hidden="true"></span>
								{displayValue}
								<span class="h-px w-8 bg-cat-3-400/80" aria-hidden="true"></span>
							</p>
						{/snippet}
					...
					<div
						class="it-rule it-d3 mx-auto mt-6 flex max-w-xs items-center gap-3"
						aria-hidden="true"
					>
						<span class="h-px flex-1 bg-on-dark/15"></span>
						<span class="size-1.5 rotate-45 border border-cat-3-400/80"></span>
						<span class="h-px flex-1 bg-on-dark/15"></span>
					</div>
					...
					<div
						class="it-item it-d4 mt-10 flex flex-wrap items-center justify-center gap-3 sm:gap-4"
					>
						<a
							href="/contact"
