<script lang="ts">
	import HeroV1 from './v1/Hero.svelte';
	import HeroV2 from './v2/Hero.svelte';
	...
	let selectedVersion = $state<'v1' | 'v2' | 'disabled'>(
		(data?.contactsHeroVersion as 'v1' | 'v2' | 'disabled') ?? 'v1'
	);
</script>
...
{#if selectedVersion !== 'disabled' || isEditable}
	...
	>
		...
		<!--
			...
			Оба варианта лежат в одной grid-ячейке (col/row-start-1), поэтому во время
			перехода уходящий и приходящий блоки накладываются друг на друга, а не
			выстраиваются в поток один под другим.
		-->
