...
<section class="relative overflow-hidden bg-gray-900 py-24 sm:py-28">
	...
	<div class="relative mx-auto max-w-7xl px-6 text-center lg:px-8">
		...
		<EditableField
			...
			label="Подзаголовок"
			value={String(data?.subtitle ?? 'Мы работаем с понедельника по субботу с 10:00 до 20:00. В иное время воспользуйтесь онлайн-чатом или почтой.')}
			{isEditable}
			multiline
		...
		>
			{#snippet children(displayValue)}
				<p class="mx-auto mt-6 max-w-2xl text-lg/8 text-gray-300">{displayValue}</p>
			{/snippet}
		</EditableField>
	</div>
</section>
