				...
				<div class="relative">
					...
					>
						{#snippet children(displayValue)}
							<h3 class="text-xl font-semibold text-gray-900">{displayValue}</h3>
						{/snippet}
					</EditableField>
					...
					<EditableField
						...
						label="Описание"
						value={String(data?.phoneDescription || 'Позвоните нам, и мы проконсультируем вас по любым вопросам')}
						{isEditable}
						multiline
					...
					>
						{#snippet children(displayValue)}
							<p class="text-base/7 text-gray-600">{displayValue}</p>
						{/snippet}
					</EditableField>
					...
					>
						{#snippet children(displayValue)}
							{#if displayValue}
								<a href="tel:{displayValue}" class="inline-flex items-center gap-2 text-lg font-semibold text-red-600 transition hover:text-red-500">
									{displayValue}
			...
			<div class="group relative overflow-hidden rounded-2xl bg-white p-8 shadow-lg ring-1 ring-gray-200 transition duration-300 hover:-translate-y-2 hover:shadow-2xl hover:ring-red-500">
				...
				<div class="relative">
					...
					<EditableField
						...
						label="Описание"
						value={String(data?.emailDescription || 'Напишите нам, и мы обязательно ответим в течение 24 часов')}
						{isEditable}
						multiline
					...
					>
						{#snippet children(displayValue)}
							{#if displayValue}
								<a href="mailto:{displayValue}" class="inline-flex items-center gap-2 text-lg font-semibold text-red-600 transition hover:text-red-500">
									{displayValue}
									<svg class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
										<path stroke-linecap="round" stroke-linejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
									</svg>
			...
			<div class="group relative overflow-hidden rounded-2xl bg-white p-8 shadow-lg ring-1 ring-gray-200 transition duration-300 hover:-translate-y-2 hover:shadow-2xl hover:ring-red-500">
				...
				<div class="relative">
					...
					<EditableField
						...
						label="Описание"
						value={String(data?.chatDescription || 'Мы готовы помочь вам в режиме реального времени')}
						{isEditable}
						multiline
					...
					>
						{#snippet children(displayValue)}
							<p class="inline-flex items-center gap-2 text-lg font-semibold text-red-600">
								{displayValue}
