...
{#if isOpen}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-scrim/50 p-4">
		<div
			class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-surface-raised p-6 shadow-xl"
		>
			<div class="mb-6 flex items-center justify-between">
				<h2 class="text-xl">Добавить проект</h2>
				<button type="button" onclick={onClose} class="text-ink-400 hover:text-ink-600">✕</button>
			</div>

			<form onsubmit={handleSubmit} class="space-y-4 text-ink-800">
				<div>
					<label for="mebel-project-category" class="mb-1 block text-sm font-medium"
						>Категория *</label
					>
					<select
						id="mebel-project-category"
						bind:value={form.category_id}
						required
						class="w-full rounded-lg border border-ink-200 p-2.5"
					>
				...
				<div>
					<label for="mebel-project-value" class="mb-1 block text-sm font-medium"
						>Название проекта *</label
					>
					<input
						id="mebel-project-value"
						type="text"
				...
				<div>
					<label for="mebel-project-short-description" class="mb-1 block text-sm font-medium"
						>Краткое описание</label
					>
					<input
						id="mebel-project-short-description"
						type="text"
				...
				<div>
					<label for="mebel-project-description" class="mb-1 block text-sm font-medium"
						>Полное описание</label
					>
					<textarea
						id="mebel-project-description"
				...
				<div class="grid grid-cols-2 gap-4">
					...
					<div>
						<label for="mebel-project-old-price" class="mb-1 block text-sm font-medium"
							>Старая цена</label
						>
						<input
							id="mebel-project-old-price"
							type="number"
				...
				<div class="pt-2">
					...
					<div class="mb-3 grid grid-cols-4 gap-3">
						{#each processedFiles as file, i}
							...
							>
								...
								<button
									...
									class="absolute top-1 right-1 rounded-full bg-brand-500 p-1 text-on-accent opacity-0 transition-opacity group-hover:opacity-100"
								>
						...
						{#if processedFiles.length < 8}
							<label
								class="flex aspect-square cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-ink-300 transition-all hover:border-link-500 hover:bg-link-50"
							>
								<svg
									class="h-6 w-6 text-ink-400"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
									><path
									...
									></path></svg
								>
								<span class="mt-1 px-1 text-center text-[10px] text-ink-500">Добавить</span>
								<input
									type="file"
									multiple
									accept="image/*"
				...
				{#if uploadProgress > 0 && uploadProgress < 100}
					<div class="mt-2 h-2 w-full rounded-full bg-ink-200">
						<div
							class="h-2 rounded-full bg-link-600 transition-all duration-300"
							style="width: {uploadProgress}%"
						></div>
					</div>
					<p class="mt-1 text-center text-[10px] text-ink-500">
						Загрузка в облако: {uploadProgress}%
					</p>
				{/if}
				...
				<div class="mt-6 flex justify-end gap-3 border-t pt-4">
					<button
						type="button"
						onclick={onClose}
						class="rounded-lg px-4 py-2 font-medium text-ink-600 hover:bg-ink-100">Отмена</button
					>
					<button
						type="submit"
						disabled={isSubmitting || processedFiles.length === 0}
						class="rounded-lg bg-link-500 px-4 py-2 font-medium text-on-accent hover:bg-link-600 disabled:opacity-50"
					>
...
{#if currentFileToCrop}
	<ImageCropper imageFile={currentFileToCrop} onCrop={handleCropped} onCancel={handleCropCancel} />
{/if}
