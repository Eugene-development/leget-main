<script lang="ts">
	import MebelBenefitsV1 from './v1/MebelBenefits.svelte';
	import MebelBenefitsV2 from './v2/MebelBenefits.svelte';
	...
	let selectedVersion = $state<'v1' | 'v2' | 'disabled'>(
		(data?.mebelBenefitsVersion as 'v1' | 'v2' | 'disabled') ?? 'v1'
	);
</script>
...
{#if selectedVersion !== 'disabled' || isEditable}
	...
	>
		<VersionSwitcher
			...
			{editContext}
			{isEditable}
			componentType="MebelBenefits"
			versionKey="mebelBenefitsVersion"
			versions={['v1', 'v2']}
			themeVersions={['v2']}
			themeDefault="light"
		...
		{#if selectedVersion === 'v2'}
			<div class="w-full" in:fly={{ x: 1200, duration: 600 }} out:fly={{ x: 1200, duration: 600 }}>
				<MebelBenefitsV2 bind:data {editContext} {isEditable} />
			</div>
		{:else}
			<div
				class="w-full"
			...
			>
				<MebelBenefitsV1 bind:data {editContext} {isEditable} />
			</div>
		{/if}
	</div>
{/if}
