<script lang="ts">
	// Артикул: 1.16.2.1 — см. docs/architecture/component-articles-map.md
	...
	async function saveField(field: string, value: string) {
		...
		await saveComponentData(editContext, 'MebelHero', updated);
		data = updated;
	}
		...
		<div class="relative px-8 py-12 sm:px-12 sm:py-16">
			...
			<h1 class="text-3xl text-on-dark sm:text-4xl lg:text-5xl">
				<EditableField
					...
					label="Заголовок"
					value={String(data.title || 'Мебель на заказ')}
					{isEditable}
					inline
			...
			<p class="mt-4 max-w-xl text-lg text-ink-300">
				<EditableField
					...
					value={String(
						data.description ||
							'Создаём уникальную корпусную мебель по вашим размерам и дизайну. Индивидуальный подход к каждому проекту.'
					)}
					{isEditable}
					inline
					multiline
			...
			<div class="mt-8 flex flex-wrap gap-4">
				...
				>
					<EditableField
						...
						value={String(data.primaryButton || 'Ваш проект')}
						{isEditable}
						inline
				...
				>
					<EditableField
						...
						value={String(data.secondaryButton || 'Бесплатный замер')}
						{isEditable}
						inline
...
<style>
	...
	@media (min-width: 1024px) {
		:global(.mebel-container) {
			padding-left: 360px; /* Space for sidebar */
		}
	}

	h1 {
		font-family: inherit; /* Use standard font from N-front */
	}
</style>
