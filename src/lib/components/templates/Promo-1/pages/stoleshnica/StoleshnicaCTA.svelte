<script lang="ts">
	// Артикул: 1.19.7.1 — см. docs/architecture/component-articles-map.md
	...
	async function saveField(field: string, value: string) {
		...
		await saveComponentData(editContext, 'StoleshnicaCTA', updated);
		data = updated;
	}
</script>
	...
	<div class="rounded-2xl bg-linear-to-r from-amber-500 to-orange-600 p-8 text-center text-white sm:p-12">
		<h2 class="text-2xl font-bold sm:text-3xl">
			<EditableField
				...
				label="Заголовок CTA"
				value={String(data.title || 'Рассчитайте стоимость столешницы')}
				{isEditable}
				inline
		...
		<div class="mt-6 flex flex-wrap justify-center gap-4">
			<button class="inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3 font-medium text-amber-600 transition-all hover:bg-amber-50">
				<EditableField
					...
					label="Текст кнопки"
					value={String(data.buttonText || 'Получить расчёт')}
					{isEditable}
					inline
