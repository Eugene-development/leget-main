<script lang="ts">
	...
	const bgImage = $derived(data.bgImage || '/plumbing_hero.png');
	...
	<section class="mebel-hero relative overflow-hidden rounded-2xl bg-slate-900">
		...
		<div class="relative px-8 py-16 sm:px-12 sm:py-20">
			<nav class="mb-6 flex items-center gap-2 text-sm text-slate-400">
				...
				<span>/</span>
				<span class="text-white">Сантехника</span>
			</nav>

			<h1 class="text-3xl font-bold text-white sm:text-4xl lg:text-5xl">
				<EditableField fieldKey="SantehnikaHero.title" label="Заголовок"
					value={String(data.title || 'Сантехника')} {isEditable} inline
			...
			<p class="mt-4 max-w-xl text-lg text-slate-300">
				<EditableField fieldKey="SantehnikaHero.description" label="Описание"
					value={String(data.description || 'Кухонные мойки, смесители, измельчители и аксессуары от ведущих производителей. Подберём идеальное сочетание цвета и формы для вашей кухни.')}
			...
			<div class="mt-8 flex flex-wrap gap-4">
				<button class="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-6 py-3 font-medium text-white transition-all hover:bg-sky-600">
					<EditableField fieldKey="SantehnikaHero.primaryButton" label="Кнопка"
						value={String(data.primaryButton || 'Подобрать комплект')} {isEditable} inline
