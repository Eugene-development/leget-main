<script lang="ts">
	// Артикул: 1.21.7.1 — см. docs/architecture/component-articles-map.md
		...
		<div class="lg:flex lg:items-center lg:gap-12">
			<div class="lg:flex-1">
				<h2 class="text-2xl text-ink-900">
					<EditableField
						fieldKey="SantehnikaComplex.title"
						label="Заголовок"
						value={String(data.title || 'Комплект для кухни')}
						{isEditable}
						inline
				...
				<p class="mt-3 leading-relaxed text-ink-600">
					<EditableField
						fieldKey="SantehnikaComplex.description"
						label="Описание"
						value={String(
							data.description ||
			...
			<div class="mt-8 flex shrink-0 lg:mt-0">
				...
				>
					<EditableField
						fieldKey="SantehnikaComplex.buttonText"
						label="Кнопка"
						value={String(data.buttonText || 'Собрать комплект')}
						{isEditable}
						inline
