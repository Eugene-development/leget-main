<script lang="ts">
	// Артикул: 1.13.2.1 — см. docs/architecture/component-articles-map.md
	...
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		<div class="max-w-2xl">
			...
			>
				{#snippet children(displayValue)}
					<p class="text-base/7 font-semibold text-cat-3-600">{displayValue}</p>
				{/snippet}
			</EditableField>
		...
		<div class="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
			{#each features as item, i}
				...
				>
					<div
						class="absolute top-6 right-6 text-7xl font-black text-ink-100 transition-colors duration-300 group-hover:text-cat-3-50"
					>
						{item.num}
					</div>
					<div class="relative">
						<div
							class="flex size-14 items-center justify-center rounded-2xl bg-ink-900 text-on-dark transition-all duration-300 group-hover:bg-cat-3-500"
						>
							<svg
								class="size-7"
								fill="none"
