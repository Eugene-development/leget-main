...
<section class="relative overflow-hidden bg-slate-950 py-24 sm:py-32">
	<div class="absolute inset-0 opacity-20">
		<div class="absolute top-0 left-0 w-full h-full bg-[radial-gradient(#8b5cf6_1px,transparent_1px)] [background-size:32px_32px]"></div>
	</div>
	<div class="relative mx-auto max-w-4xl px-6 text-center lg:px-8">
		...
		<EditableField
			...
			label="Описание"
			value={String(data?.description ?? 'Закажите профессиональный замер — первый шаг к идеальному интерьеру')}
			{isEditable}
			multiline
		...
		<div class="mt-12">
			...
			>
				{#snippet children(displayValue)}
					<button
						type="button"
						class="inline-flex items-center gap-3 rounded-full bg-white px-10 py-5 text-base font-bold text-slate-950 shadow-2xl transition-all duration-300 hover:scale-105 hover:bg-violet-500 hover:text-white active:scale-95"
					>
						{displayValue}
					</button>
				{/snippet}
