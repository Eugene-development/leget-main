		...
		<div class="max-w-2xl">
			...
			>
				{#snippet children(displayValue)}
					<p class="text-base/7 font-semibold text-orange-600">{displayValue}</p>
				{/snippet}
			</EditableField>
		...
		<div class="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
			{#each features as item, i}
				<div class="group relative overflow-hidden rounded-3xl border border-slate-100 bg-slate-50/50 p-8 transition-all duration-300 hover:-translate-y-2 hover:bg-white hover:shadow-2xl">
					<div class="absolute right-6 top-6 text-7xl font-black text-slate-100 transition-colors duration-300 group-hover:text-orange-50">{item.num}</div>
					<div class="relative">
						<div class="flex size-14 items-center justify-center rounded-2xl bg-slate-900 text-white transition-all duration-300 group-hover:bg-orange-500">
