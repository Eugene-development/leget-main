...
<section class="relative h-[calc(100vh-88px)] flex items-center overflow-hidden bg-slate-950">
	<div class="absolute inset-0 z-0">
		<div class="absolute top-0 right-0 w-[800px] h-[800px] bg-orange-500/10 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/4"></div>
		...
		<div class="grid items-center gap-12 lg:grid-cols-12 lg:gap-20">
			<div class="lg:col-span-5" in:fly={{ x: -50, duration: 1000, easing: cubicOut }}>
				...
				<h1 class="text-4xl font-extrabold tracking-tight text-white sm:text-6xl lg:text-7xl leading-[1.1]">
					...
					>
						{#snippet children(displayValue)}
							<span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-400">{displayValue}</span>
						{/snippet}
					</EditableField>
				</h1>
				...
				<EditableField
					...
					label="Описание"
					value={String(data?.description ?? 'Индивидуальная мебель, спроектированная под ваше пространство с учётом ваших пожеланий. От стартового эскиза до рабочих чертежей для производства.')}
					{isEditable}
					multiline
					...
					>
						{#snippet children(displayValue)}
							<button
								...
								class="group relative inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-orange-500 to-amber-600 px-8 py-4 text-base font-bold text-white shadow-xl shadow-orange-500/20 transition-all duration-300 hover:scale-105 hover:shadow-orange-500/40 active:scale-95 cursor-pointer"
							>
			...
			<div class="relative hidden lg:col-span-7 lg:justify-self-end lg:block w-full" in:fly={{ x: 50, duration: 1000, delay: 200, easing: cubicOut }}>
				<div class="absolute -inset-10 bg-gradient-to-tr from-orange-500/20 to-amber-500/20 blur-[80px] opacity-50"></div>
				<div class="relative group">
					<div class="relative z-10 overflow-hidden rounded-[2.5rem] border border-white/10 shadow-2xl backdrop-blur-3xl bg-white/5 p-4 transition-transform duration-500 group-hover:scale-[1.02]">
						<img
							src={String(data?.hero_image ?? '/wardrobe_project_no_text.png')}
	...
	<div class="absolute bottom-10 left-1/2 -translate-x-1/2 z-20">
		...
		>
			<svg 
				class="w-10 h-10 text-white/20 group-hover:text-orange-400 transition-colors" 
				fill="none" 
				viewBox="0 0 24 24" 
				stroke="currentColor" 
				stroke-width="2"
