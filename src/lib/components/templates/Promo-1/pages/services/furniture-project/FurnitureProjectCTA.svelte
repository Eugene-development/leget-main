<script lang="ts">
	// Артикул: 1.14.4.1 — см. docs/architecture/component-articles-map.md
...
<section class="relative overflow-hidden bg-ink-950 py-24 sm:py-32">
	<div class="absolute inset-0 opacity-20">
		<div
			class="absolute top-0 left-0 h-full w-full bg-[radial-gradient(var(--color-cat-8-500)_1px,transparent_1px)] [background-size:32px_32px]"
		></div>
	</div>
		...
		<div class="mt-12">
			...
			>
				{#snippet children(displayValue)}
					<button
						type="button"
						class="inline-flex items-center gap-3 rounded-full bg-surface-raised px-10 py-5 text-base font-bold text-ink-950 shadow-2xl transition-all duration-300 hover:scale-105 hover:bg-cat-8-500 hover:text-on-accent active:scale-95"
					>
						{displayValue}
					</button>
				{/snippet}
