<script lang="ts">
	// Артикул: 1.12.3.1 — см. docs/architecture/component-articles-map.md
	...
	async function saveField(field: string, value: string) {
		...
		await saveComponentData(editContext, 'DesignProjectWhy', updated);
		data = updated;
	}
...
<section class="bg-ink-50 py-24 sm:py-32">
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		<div class="grid items-center gap-16 lg:grid-cols-2">
			...
			<div>
				<EditableField
					fieldKey="DesignProjectWhy.title"
					label="Заголовок"
					value={String(data?.title ?? 'Почему нужен дизайн-проект?')}
					{isEditable}
					...
					{#each reasons as item, i}
						<div class="group flex gap-8">
							...
							<dd class="pt-1">
								...
								<EditableField
									fieldKey={`DesignProjectWhy.reason_${i}_desc`}
