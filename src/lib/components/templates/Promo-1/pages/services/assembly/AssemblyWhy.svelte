...
<section class="bg-slate-50 py-24 sm:py-32">
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		<div class="grid items-center gap-16 lg:grid-cols-2">
			...
			<div class="relative">
				<div
					class="absolute -right-6 -bottom-6 size-48 rounded-[2.5rem] bg-rose-500 lg:size-64"
				></div>
				<div class="absolute -top-4 -left-4 size-32 rounded-full bg-rose-500/30 blur-2xl"></div>
				<div
					class="relative overflow-hidden rounded-[2.5rem] border border-white/20 shadow-2xl transition duration-500 hover:scale-105"
				>
					<img loading="lazy"
						src={String(data?.imageUrl ?? '/furniture_assembly_process.png')}
						alt="Сборка мебели"
				...
				<dl class="mt-12 space-y-10">
					{#each reasons as item, i}
						<div class="group flex gap-8">
							<dt
								class="text-5xl font-black text-rose-500/20 transition-colors duration-300 group-hover:text-rose-500"
							>
								{item.num}
							</dt>
							<dd class="pt-1">
