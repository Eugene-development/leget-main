...
<section class="relative flex h-[calc(100vh-88px)] items-center overflow-hidden bg-ink-950">
	<div class="absolute inset-0 z-0">
		<div
			class="absolute top-0 right-0 h-[800px] w-[800px] translate-x-1/4 -translate-y-1/2 rounded-full bg-cat-6-500/10 blur-[120px]"
		></div>
		<div
	...
	<div class="relative z-10 mx-auto w-full max-w-7xl px-6 lg:px-8">
		<div class="grid items-center gap-12 lg:grid-cols-12 lg:gap-20">
			<div class="lg:col-span-6" in:fly={{ x: -50, duration: 1000, easing: cubicOut }}>
				...
				>
					{#snippet children(displayValue)}
						<div
							class="inline-flex items-center gap-2 rounded-full border border-cat-6-500/20 bg-cat-6-500/10 px-3 py-1 text-sm font-medium text-cat-6-400"
						>
							<span class="relative flex h-2 w-2">
								<span
									class="absolute inline-flex h-full w-full animate-ping rounded-full bg-cat-6-400 opacity-75"
								></span>
								<span class="relative inline-flex h-2 w-2 rounded-full bg-cat-6-500"></span>
							</span>
							{displayValue}
						</div>
					{/snippet}
				...
				<h1 class="text-4xl leading-[1.1] tracking-tight text-on-dark sm:text-6xl lg:text-7xl">
					...
					>
						{#snippet children(displayValue)}
							<span
								class="bg-gradient-to-r from-cat-6-400 to-cat-7-400 bg-clip-text text-transparent"
								>{displayValue}</span
							>
						{/snippet}
					</EditableField>
				...
				<div class="mt-12 flex flex-wrap gap-6">
					...
					>
						{#snippet children(displayValue)}
							<button
								type="button"
								onclick={() => serviceOrderStore.open('assembly')}
								class="group relative inline-flex cursor-pointer items-center gap-3 rounded-full bg-gradient-to-r from-cat-6-500 to-cat-7-600 px-8 py-4 text-base font-bold text-on-accent shadow-xl shadow-cat-6-500/20 transition-all duration-300 hover:scale-105 hover:shadow-cat-6-500/40 active:scale-95"
							>
			...
			<div
				class="relative hidden w-full lg:col-span-6 lg:block lg:justify-self-end"
				in:fly={{ x: 50, duration: 1000, delay: 200, easing: cubicOut }}
			>
				<div
					class="absolute -inset-10 bg-gradient-to-tr from-cat-6-500/20 to-cat-7-500/20 opacity-50 blur-[80px]"
				></div>
	...
	<div class="absolute bottom-10 left-1/2 z-20 -translate-x-1/2">
		...
		>
			<svg
				class="h-10 w-10 text-on-dark/20 transition-colors group-hover:text-cat-6-400"
				fill="none"
				viewBox="0 0 24 24"
				stroke="currentColor"
				stroke-width="2"
