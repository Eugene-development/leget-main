<script lang="ts">
	// Артикул: 1.28.2.1 — см. docs/architecture/component-articles-map.md
	//
	// Шапка страницы бренда (/bytovaya-tehnika/{brand}). Заголовок и описание
	...
	async function saveField(field: string, value: string) {
		...
		await saveComponentData(editContext, 'ByttehnikaBrandHero', updated);
		data = updated;
	}
</script>
	...
	<section class="relative overflow-hidden rounded-2xl bg-ink-900">
		...
		<div class="relative px-8 py-12 sm:px-12 sm:py-16">
			<nav class="mb-6 flex items-center gap-2 text-sm text-ink-400">
				...
				<span>/</span>
				<a href="/bytovaya-tehnika" class="transition-colors hover:text-on-dark">Бытовая техника</a>
				<span>/</span>
			...
			<h1 class="text-3xl text-on-dark sm:text-4xl lg:text-5xl">
				<EditableField
					fieldKey="ByttehnikaBrandHero.title"
					label="Заголовок"
					value={title}
					{isEditable}
					inline
			...
			{#if data.description}
				<p class="mt-4 max-w-xl text-lg text-ink-300">
					<EditableField
						fieldKey="ByttehnikaBrandHero.description"
						label="Описание"
			...
			<div class="mt-8 flex flex-wrap gap-4">
				...
				>
					<EditableField
						fieldKey="ByttehnikaBrandHero.buttonText"
						label="Текст кнопки"
						value={String(data.buttonText || 'Подобрать технику')}
						{isEditable}
						inline
...
<style>
	/* Полоса контента страницы бренда. Объявлена через :global так же, как у
	   ByttehnikaHero и пяти остальных Hero каталожных страниц: остальные блоки
