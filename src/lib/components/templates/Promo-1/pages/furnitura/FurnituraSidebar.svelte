...
<aside bind:this={sidebarElement} class="mebel-sidebar hidden lg:block" style="top: {offsetTop}px;">
	<div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
		<h3 class="mb-6 text-lg font-medium text-slate-900">Магазины фурнитуры</h3>
		...
		<nav class="space-y-1">
			{#each shops as shop}
				...
				>
					<span class="font-medium">{shop.title ?? shop.value ?? shop.name}</span>
		...
		<div class="mt-8 rounded-xl bg-slate-800 p-6">
			<h4 class="font-bold text-white">Премиум фурнитура</h4>
			<p class="mt-2 text-sm leading-relaxed text-slate-300">
				Blum, Hettich, Grass — официальные дилеры
			</p>
			<button class="mt-4 w-full rounded-lg bg-amber-500 py-2.5 text-sm font-semibold text-white transition-all hover:bg-amber-600">
				Подобрать фурнитуру
			</button>
		</div>
	</div>
