<script lang="ts">
	// Артикул: 1.23.2.1 — см. docs/architecture/component-articles-map.md
...
<div class="mebel-container">
	<section class="mebel-hero relative overflow-hidden rounded-2xl bg-ink-900">
		...
		{:else}
			...
			<div class="absolute inset-0 bg-gradient-to-br from-cat-1-900/30 to-ink-900/80"></div>
		{/if}
		...
		<div class="relative px-8 py-16 sm:px-12 sm:py-20">
			<nav class="mb-6 flex items-center gap-2 text-sm text-ink-400">
				...
				<span>/</span>
				<span class="text-on-dark">Плитка</span>
			</nav>

			<h1 class="text-3xl text-on-dark sm:text-4xl lg:text-5xl">
				<EditableField
					fieldKey="PliitkaHero.title"
					label="Заголовок"
					value={String(data.title || 'Плитка')}
					{isEditable}
					inline
			...
			<p class="mt-4 max-w-xl text-lg text-ink-300">
				<EditableField
					fieldKey="PliitkaHero.description"
					label="Описание"
					value={String(
						data.description ||
							'Керамическая плитка, керамогранит и мозаика от ведущих мировых производителей. Подберём оптимальное решение для любого интерьера.'
					)}
					{isEditable}
					inline
					multiline
			...
			<div class="mt-8 flex flex-wrap gap-4">
				...
				>
					<EditableField
						fieldKey="PliitkaHero.primaryButton"
						label="Кнопка"
						value={String(data.primaryButton || 'Подобрать плитку')}
						{isEditable}
						inline
