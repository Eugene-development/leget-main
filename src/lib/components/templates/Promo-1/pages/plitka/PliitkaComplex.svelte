		...
		<div class="lg:flex lg:items-center lg:gap-12">
			<div class="lg:flex-1">
				<h2 class="text-2xl font-bold text-slate-900">
					<EditableField fieldKey="PliitkaComplex.title" label="Заголовок"
						value={String(data.title || 'Комплексное решение')} {isEditable} inline
						onSave={(v) => saveField('title', v)}>
						{#snippet children(val)}{val}{/snippet}
					</EditableField>
				</h2>
				<p class="mt-3 leading-relaxed text-slate-600">
					<EditableField fieldKey="PliitkaComplex.description" label="Описание"
						value={String(data.description || 'Закажите плитку вместе с дизайн-проектом и получите скидку на весь комплект. Наши дизайнеры подберут плитку, которая идеально впишется в ваш интерьер.')}
			...
			<div class="mt-8 flex shrink-0 lg:mt-0">
				<button class="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-6 py-3 font-medium text-white transition-all hover:bg-sky-600">
					<EditableField fieldKey="PliitkaComplex.buttonText" label="Кнопка"
						value={String(data.buttonText || 'Заказать дизайн-проект')} {isEditable} inline
