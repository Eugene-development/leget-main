<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			const response = await fetch('/goals/session', {
				method: 'POST',
			...
			await goto('/goals', { replaceState: true });
		} catch {
...
<svelte:head>
	<title>Открываем карту целей</title>
	<meta name="robots" content="noindex, nofollow" />
</svelte:head>

<main class="flex min-h-screen items-center justify-center bg-surface-warm px-6 text-text-primary">
	{#if errorMessage}
		<div class="max-w-md text-center" role="alert">
			<h1 class="text-3xl">Не удалось открыть карту целей</h1>
	...
	{:else}
		<div class="flex items-center gap-3" role="status" aria-live="polite">
			...
			<span class="text-sm text-text-secondary">Проверяем доступ к карте целей…</span>
		</div>
	{/if}
</main>
