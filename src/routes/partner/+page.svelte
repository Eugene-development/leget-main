<script lang="ts">
	...
	/** Первые буквы имени — заглушка вместо аватара, файлов у партнёра пока нет. */
...
<main class="min-h-screen bg-surface-warm px-4 py-12 sm:px-6 lg:px-8">
	<div class="mx-auto max-w-3xl">
		...
		{:else if profile}
			...
			>
				<div class="border-b border-border-light px-7 py-5">
					...
					<p class="mt-1 text-sm text-ink-500">
						Профиль общий с личным кабинетом — отличается только доступ к разделам.
					</p>
				</div>
				...
				<dl class="divide-y divide-border-light">
					...
					<div class="grid gap-1 px-7 py-4 sm:grid-cols-3 sm:gap-4">
						...
						<dd class="text-sm text-ink-950 sm:col-span-2">Партнёр</dd>
					</div>
					<div class="grid gap-1 px-7 py-4 sm:grid-cols-3 sm:gap-4">
			...
			<!--
				Пустое состояние без выдуманных цифр: партнёрских данных в схеме
				ещё нет (ни рефералов, ни начислений), и показать «0 ₽» значило бы
				соврать про существующий, но нулевой счёт.
			-->
			<section
				class="mt-6 rounded-3xl border border-dashed border-border-medium bg-surface-muted p-7"
			>
				<h2 class="text-xl text-ink-950">Партнёрская программа</h2>
				<p class="mt-2 text-sm leading-relaxed text-ink-600">
					Раздел готовится. Доступ к кабинету уже ваш — привлечённые клиенты, начисления и документы
					появятся здесь, как только заработает учёт.
				</p>
				<p class="mt-4 text-sm text-ink-500">
					Вопросы по программе — на почту
					<a class="text-link-600 underline underline-offset-2" href="mailto:info@leget.ru">
						info@leget.ru
					</a>.
				</p>
			</section>
